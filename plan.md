...
- [ ] Pass 1 catalog: CREATE/ALTER TABLE, CREATE INDEX, PK/UQ constraints,
      collations, temp tables, table variables.
- [ ] Precedence matrix + literal typing rules encoded, with unit tests per
      type pair we intend to report (start narrow: varchar/nvarchar,
      char/nchar, int/varchar, datetime/varchar).
- [ ] Tier-1 syntactic rules (no types needed): function-on-column,
...
- [ ] View dependency graph, topological resolution, cycle → UNKNOWN.
- [ ] Column provenance: BaseColumn / Expression / Cast / Unknown.
- [ ] SELECT * expansion, aliases, UNION type unification (record all branch
      types), inline TVFs, nested subqueries in FROM.
- [ ] Verify oracle: deploy each fixture to Docker, diff inferred view column
...
- [ ] Extract comparisons from WHERE/ON/HAVING in procs/views/functions.
- [ ] Resolve column side through lineage; type the other side (literal,
      @param/@variable declarations, other column).
- [ ] Verdict engine incl. collation rules (SQL_* scan vs Windows RANGE_SEEK)
      and column-side-only direction logic; depth + origin attribution
      ("mismatch introduced by CAST in vw_X line 12").
- [ ] Dynamic SQL bucketing (unanalyzable counter).
...
- [ ] Corpus manifest; pick 5 open-source projects with real DDL+procs in-repo.
- [ ] Scan → hand-verify every finding → oracle-confirm via Docker deploy +
      plan XML probe.
- [ ] Measure precision; fix rules until >95% on this set. Record UNKNOWN and
...
- [ ] Synthetic tables per reported type pair; 10K/1M/10M rows; matched vs
...
- [ ] Broaden corpus (top open-source .NET/SQL Server apps + GitHub sweep with
      dialect sniffing). Pin SHAs.
- [ ] Run, oracle-confirm the headline subset, aggregate stats incl. depth
      distribution (the unique number: % of findings inherited through views).
- [ ] File fixes/issues with maintainers FIRST; wait a courtesy window.
- [ ] Writeup: name the phenomenon, lead with the two numbers, methodology
...
- [ ] Ship order: blog post + Show HN + the tool (pip-adjacent UX: single
      dotnet tool install, one command, pretty output).
